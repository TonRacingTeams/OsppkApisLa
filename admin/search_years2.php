...
<div id='showa' class="table-responsive p-10">
                ...
                if($start=="" or $end==""){$btw="";}
                else{ $btw="and KHT_Decision_AY.Item_Date between '$start' and '$end'";}


                if($Item_ID==""){$a="";}
                else{ $a="and (KHT_Decision_AY.Item_ID like N'%$Item_ID%' or KHT_Decision_AY.Item_ID like N'$Item_ID%')";}


                if($Deci_No==""){$b="";}
                else{ $b="and (KHT_Decision_AY.Deci_No like N'%$Deci_No%' or KHT_Decision_AY.Deci_No like N'$Deci_No%')";}



                if($Full_Name==""){$c="";}
                else{ $c="and (KHT_Decision_AY.Full_Name like N'%$Full_Name%' or KHT_Decision_AY.Full_Name like N'$Full_Name%')";}


                $i=1;
                $sql = "SELECT * FROM KHT_Decision_AY WHERE 1=1 $btw $a $b $c" ;
...
<tbody id="users">
           ...
           <td align='center'>
           <a href="pages/Delete_FrmRptCrm_Yearly2.php?Item_ID=<?php echo $result['Item_ID']?>" class="btn btn-danger"  onclick=" return confirm('ທານຕ້ອງການລົບຂໍ້ມູນນີ້ແທ້ ຫຼື ບໍ..?')"><i class="fas fa-trash fa-sm"></i></a>
           </td>
                        ...
                        <?php
                        $dates=$result["Date_Jub"];
                        ?>
                        <td><?PHP echo date_format($dates,'d/m/Y');?></td>


                        <td><?PHP echo $result["Kankatham"]; ?></td>

                        <td><?PHP echo $result["Data_holding"]; ?></td>
                        <td><?PHP echo $result["Santad"]; ?></td>
